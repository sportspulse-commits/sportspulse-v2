...
function normalizeBet(b: any) {
  ...
  return { ...b, net_profit, date, game, pick, league, betType, stake, payout };
}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {recentBets.length > 0 && (
                <div style={{ background: '#0f1629', border: '1px solid #1e3a5f', borderRadius: '8px', padding: '20px' }}>
                  ...
                  <div style={{ overflowX: 'auto' as const }}><table style={{ width: '100%', borderCollapse: 'collapse' as const, fontSize: '11px' }}><thead><tr>{['Date','Game','Pick','Odds','Stake','Result','P&L'].map(function(h) { return <th key={h} style={{ color: '#475569', textAlign: 'left' as const, padding: '6px 8px', borderBottom: '1px solid #1e3a5f', letterSpacing: '1px' }}>{h}</th>; })}</tr></thead><tbody>{recentBets.map(function(bet: any) { const rc = bet.status === 'won' ? '#0d2a1a' : bet.status === 'lost' ? '#2a0d0d' : 'transparent'; const pc = bet.pnl > 0 ? '#22c55e' : bet.pnl < 0 ? '#ef4444' : '#475569'; return (<tr key={bet.id} style={{ background: rc, borderBottom: '1px solid #1e3a5f' }}><td style={{ padding: '8px', color: '#475569' }}>{bet.date}</td><td style={{ padding: '8px', color: '#94a3b8', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{bet.game}</td><td style={{ padding: '8px', color: '#e2e8f0', fontWeight: 'bold' }}>{bet.pick}</td><td style={{ padding: '8px', color: '#94a3b8' }}>{bet.odds > 0 ? '+' + bet.odds : bet.odds}</td><td style={{ padding: '8px', color: '#94a3b8' }}>{'$' + Number(bet.stake).toFixed(2)}</td><td style={{ padding: '8px', color: bet.status === 'won' ? '#22c55e' : bet.status === 'lost' ? '#ef4444' : '#475569', fontWeight: 'bold', textTransform: 'uppercase' as const }}>{bet.status}</td><td style={{ padding: '8px', color: pc, fontWeight: 'bold' }}>{bet.pnl >= 0 ? '+$' : '-$'}{Math.abs(bet.pnl).toFixed(2)}</td></tr>); })}</tbody></table></div>
                </div>
              )}
