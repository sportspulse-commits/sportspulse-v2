...
export async function GET(request: Request) {
  ...
  const pnlOverTime = settled.map(function(b, idx) {
    ...
    running += gain;
    return { idx: idx + 1, date: (b.settled_at || b.created_at || '').slice(0, 10), settledAt: b.settled_at || b.created_at || '', pnl: Math.round(running * 100) / 100, gain: Math.round(gain * 100) / 100, game: (b.away_team || '') + ' @ ' + (b.home_team || ''), pick: b.bet_pick || '', odds: b.odds || 0, stake: Number(b.stake || 0), result: b.status };
  });
  ...
  }).filter(function(l) { return l.total > 0; }).sort(function(a, b) { return b.winRate !== a.winRate ? b.winRate - a.winRate : a.league.localeCompare(b.league); });
  ...
  }).filter(function(t) { return t.total > 0; }).sort(function(a, b) { return b.winRate !== a.winRate ? b.winRate - a.winRate : a.type.localeCompare(b.type); });
  ...
  const recentBets = [...settled].reverse().slice(0, 50).map(function(b) {
    return { id: b.id, date: (b.settled_at || b.created_at || '').slice(0, 10), settledAt: b.settled_at || b.created_at || '', game: (b.away_team || '') + ' @ ' + (b.home_team || ''), pick: b.bet_pick || '', odds: b.odds || 0, stake: Number(b.stake || 0), status: b.status, pnl: Math.round(Number(b.net_profit || 0) * 100) / 100, league: (b.league || '').toUpperCase(), betType: b.bet_type || '' };
  });
