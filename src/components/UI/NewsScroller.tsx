...
export default function NewsScroller({ articles: passedArticles = [] }: { articles?: any[] }) {
  ...
  useEffect(function() {
    ...
    function animate() {
      ...
      if (!isPaused) {
        ...
        var half = elem.scrollWidth / 2;
        if (Math.abs(posRef.current) >= half) { posRef.current = 0; }
        elem.style.transform = 'translateX(' + posRef.current + 'px)';
      }
  ...
  var wrapStyle = { position: 'fixed' as const, bottom: 0, left: 0, right: 0, height: '36px', background: '#0a0e1a', borderTop: '1px solid #1e3a5f', zIndex: 997, overflow: 'hidden', display: 'flex', alignItems: 'center' };
  var trackStyle = { display: 'flex', alignItems: 'center', height: '100%', whiteSpace: 'nowrap' as const, willChange: 'transform' as const };
  return (
    React.createElement('div', { style: wrapStyle, onMouseEnter: function() { setIsPaused(true); }, onMouseLeave: function() { setIsPaused(false); } },
      React.createElement('div', { style: { overflow: 'hidden', flex: 1, height: '100%', position: 'relative' as const } },
